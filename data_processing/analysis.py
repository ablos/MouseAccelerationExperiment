...
agg = trials.groupby(["participant_id", "session_number", "task_type", "group"]).agg(
...
baseline = agg[agg["session_number"] == 1].set_index(["participant_id", "task_type"])
...
for metric, label in metrics:
    ...
    for ax, task in zip(axes, ["clicking", "dragging", "slider"]):
        ...
        sns.lineplot(data=task_data, x="session_number", y=metric, hue="group", ax=ax, marker="o",
                     palette={"control": "steelblue", "experimental": "tomato"})
        sns.regplot(data=task_data[task_data["group"] == "control"], x="session_number", y=metric,
            ax=ax, scatter=False, color="steelblue", line_kws={"linestyle": "--", "alpha": 0.5})
        sns.regplot(data=task_data[task_data["group"] == "experimental"], x="session_number", y=metric,
